...
- [ ] **EDIT-01**: 用户在隐藏路由输入正确密码后可进入编辑界面；密码错误或未输入则留在入口页，不暴露任何编辑能力
- [ ] **EDIT-02**: 编辑界面加载时读取现有内容集合（经 lib/anime.ts / lib/tools.ts 收口），展示可编辑条目列表

### 内容编辑 (Edit)

- [ ] **EDIT-03**: 维护者可编辑已有追番条目，字段包括：状态（在看 / 看完 / 想看）、观看进度（看到第 N 话 / 共 M 话）、评分（0-10）、短评、封面 URL
- [ ] **EDIT-04**: 维护者可编辑已有工具条目，字段包括：名称、一句话描述、外链 URL、标签列表、分类、正文（可长可短的图文内容）

### 新增条目 (Create)

- [ ] **EDIT-05**: 维护者可在站内新增追番条目，表单带必填校验（名称、状态为必填；集数、评分可选）
- [ ] **EDIT-06**: 维护者可在站内新增工具条目，表单带必填校验（名称、分类为必填；描述、链接、标签可选）
- [ ] **EDIT-07**: 新建条目时自动生成合规 frontmatter（slug 派生自名称、默认状态/空字段、字段类型与 content.config.ts Zod schema 一致）

### 数据导出 (Export)

- [ ] **EDIT-08**: 编辑/新增结果可下载为 JSON 文件，格式完全匹配 content.config.ts schema；下载后放进 src/content/anime/ 或 src/content/tools/ 即可触发既有 git → Vercel 部署链路
...
v1.1 需求到 roadmap 阶段的映射。roadmap 创建后填充。
...
*Last updated: 2026-07-16 after milestone v1.1 start*
