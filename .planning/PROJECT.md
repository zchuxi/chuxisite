...
### Validated (v1.1 Phase 4)

- ✓ **EDIT-01**: 访客在隐藏路由输入正确密码后可进入编辑界面（前端密码守卫，失败留在入口页）— Phase 4
- ✓ **EDIT-02**: 工作台读取现有条目并展示（经 lib/anime.ts / lib/tools.ts 收口），支持选择要编辑的条目 — Phase 4

### Active（v1.1 Phase 5-7 — 内容编辑 UI 续）

- [ ] **EDIT-03**: 访客在隐藏路由输入正确密码后可进入编辑界面（前端密码守卫，失败留在入口页）
- [ ] **EDIT-04**: 维护者可编辑已有追番条目（状态 / 进度 / 评分 / 短评 / 封面 URL）
- [ ] **EDIT-05**: 维护者可编辑已有工具条目（名称 / 描述 / 链接 / 标签 / 分类 / 正文）— Phase 5
- [ ] **EDIT-06**: 维护者可新增追番条目（表单 + 验证）— Phase 6
- [ ] **EDIT-07**: 维护者可新增工具条目（表单 + 验证）— Phase 6
- [ ] **EDIT-08**: 新项目创建时生成合规 frontmatter（slug 派生、默认值、必填校验）— Phase 6
- [ ] **EDIT-09**: 编辑结果可下载为 schema 兼容的 JSON 文件，放进 content/ 走 git 部署 — Phase 7
...
---
*Last updated: 2026-07-16 — Phase 4 (Admin shell) complete; Phase 5 (编辑已有条目) next*
